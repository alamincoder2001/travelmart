    ...
    public function getCashLedger() {
        ...
        $ledger = $this->db->query("
            ...
            select 
                bm.id,
                bm.date,
                concat('Bill - ', bm.invoice, ' - ', c.Customer_Name, ' (', c.Customer_Code, ')', ' - Amount: ', bm.total) as description,
                bm.paid as in_amount,
                0.00 as out_amount
            from tbl_billmaster bm 
            join tbl_customer c on c.Customer_SlNo = bm.client_id
            where bm.status = 'a'
            and bm.branch_id = '$this->brunch'
            and bm.date between '$data->fromDate' and '$data->toDate'
            ...
            select 
                pm.id,
                pm.date,
                concat('Purchase - ', pm.invoice, ' - ', s.Supplier_Name, ' (', s.Supplier_Code, ')', ' - Bill: ', pm.purchase_total) as description,
                0.00 as in_amount,
                0.00 as out_amount
            from tbl_billmaster pm 
            join tbl_supplier s on s.Supplier_SlNo = pm.supplier_id
            where pm.status = 'a'
            and pm.branch_id = '$this->brunch'
            and pm.date between '$data->fromDate' and '$data->toDate'
