...
class Model_Table extends CI_Model{
    ...
    public function generateBillInvoice(){
        ...
        $invoice = date('y') . $branchNo . "00001";
        $year = date('y');
        $bill = $this->db->query("select * from tbl_billmaster bm where bm.invoice like '$year%' and branch_id = ?", $branchId);
        if($bill->num_rows() != 0){
            $newBillId = $bill->num_rows() + 1;
            $zeros = array('0', '00', '000', '0000');
            $invoice = date('y') . $branchNo . (strlen($newBillId) > count($zeros) ? $newBillId : $zeros[count($zeros) - strlen($newBillId)] . $newBillId);
		}
    ...
    public function getTransactionSummary($date = null){
        $transactionSummary = $this->db->query("
            ...
            (
                select ifnull(sum(bm.paid), 0) from tbl_billmaster bm
                where bm.branch_id= " . $this->session->userdata('BRANCHid') . "
                and bm.status = 'a'
            ...
            /* total */
            (
                select received_sales + received_bills + received_customer + received_supplier + received_cash + bank_withdraw + loan_received + loan_initial_balance + invest_received + sale_asset
            ) as total_in,
            (
    ...
    public function supplierDue($clauses = "" , $date = null) {
        ...
        $supplierDues = $this->db->query("
            ...
            s.Supplier_Address,
            s.contact_person,
            (select ifnull(sum(bm.purchase_total), 0.00) from tbl_billmaster bm
                where bm.supplier_id = s.Supplier_SlNo
                " . ($date == null ? "" : " and bm.date < '$date'") . "
                and bm.status = 'a'
            ) as bill,
    ...
    public function clientDue($clauses = "" , $date = null) {
        $branchId = $this->session->userdata('BRANCHid');
        $dueResult = $this->db->query("
            select
            c.Customer_SlNo,
            ...
            c.Customer_Address,
            c.Customer_Mobile,
            (select ifnull(sum(bm.total), 0.00) + ifnull(c.previous_due, 0.00)
                from tbl_billmaster bm 
                where bm.client_id = c.Customer_SlNo
                " . ($date == null ? "" : " and bm.date < '$date'") . "
                and bm.status = 'a') as billAmount,

            (select ifnull(sum(bm.paid), 0.00)
                from tbl_billmaster bm
                where bm.client_id = c.Customer_SlNo
                " . ($date == null ? "" : " and bm.date < '$date'") . "
                and bm.status = 'a') as invoicePaid,
