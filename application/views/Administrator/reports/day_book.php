					...
					<div style="width:50%;border:1px solid black;position:relative;">
						<table class="day-book-table">
							...
							<tbody>
								...
								<template v-if="sales.length > 0" style="display:none;">
									<tr>
								...
								<template v-if="bills.length > 0">
									<tr>
										<td class="sub-heading">Bills</td>
										<td class="sub-value">{{ totalBills }}</td>
									</tr>
									<tr v-for="bill in bills">
										<td>{{ bill.Customer_Name }}</td>
										<td>{{ bill.totalAmount | decimal }}</td>
									</tr>
								</template>
								...
								<template v-if="receivedFromCustomers.length > 0">
									<tr>
										<td class="sub-heading">Client Payment</td>
						...
						<table class="day-book-table">
							...
							<tbody>
								...
								<template v-if="paidToCustomers.length > 0">
									<tr>
										<td class="sub-heading">Paid to Clients</td>
...
<script>
	new Vue({
		...
		methods: {
			...
			getBills() {
				axios.post('/get_bills', this.filter)
					.then(res => {
						let bills = res.data.bills.filter(bill => bill.paid > 0);
						bills = _.groupBy(bills, 'client_id');
						bills = _.toArray(bills);
						bills = bills.map(bill => {
							bill[0].totalAmount = bill.reduce((p, c) => { return p + parseFloat(c.paid) }, 0);
							return bill[0];
						})
						this.bills = bills;
					})
