		...
		<div class="col-md-12">
			<form class="form-inline" v-on:submit.prevent="getProfitLoss">
				<div class="form-group" style="margin-right: 15px;">
					<label>Client &nbsp;</label>
					<v-select v-bind:options="customers" v-model="selectedCustomer" label="display_name" placeholder="Select Client"></v-select>
				</div>
				...
				<table id="table1">
					...
					<tfoot style="font-weight:bold;background-color:#e9dcdc;">
							...
							<td style="text-align:right;">
								{{ 
									reportData.reduce((prev, cur) => { return prev + parseFloat(
										cur.billDetails.reduce((p, c) => { return p + parseFloat(c.purchased_amount) }, 0)
									)}, 0).toFixed(2)
								}}
							</td>
							<td style="text-align:right;">
							...
							<td style="text-align:right;">
								{{ 
									totalProfit = reportData.reduce((prev, cur) => { return prev + parseFloat(
										cur.billDetails.reduce((p, c) => { return p + parseFloat(c.profit_loss) }, 0)
									)}, 0).toFixed(2)
								}}
							</td>
						</tr>
						...
						<tr>
							...
							<td colspan="2"></td>
							<td style="text-align:right;">{{ totalVat = reportData.reduce((prev, cur) => { return prev + parseFloat(cur.vat) }, 0).toFixed(2) }}</td>
						</tr>

						<tr>
							<td colspan="3" style="text-align:right;">Total Discount (-)</td>
							<td colspan="2"></td>
							<td style="text-align:right;">{{ totalDiscount = reportData.reduce((prev, cur) => { return prev + parseFloat(cur.discount) }, 0).toFixed(2) }}</td>
						</tr>

						<tr style="display: none;">
							<td colspan="3" style="text-align:right;">Total Returned Value (-)</td>
							<td colspan="2"></td>
						...
						<tr>
							<td colspan="3" style="text-align:right;">Profit Distribute (-)</td>
							<td colspan="2"></td>
							<td style="text-align:right;">{{ otherIncomeExpense.profit_distribute | decimal }}</td>
						</tr>

						<tr>
							<td colspan="3" style="text-align:right;">Loan Interest (-)</td>
							<td colspan="2"></td>
							<td style="text-align:right;">{{ otherIncomeExpense.loan_interest | decimal }}</td>
						</tr>

						<tr>
							<td colspan="3" style="text-align:right;">Assets Sales | Profit/Loss (-)</td>
							<td colspan="2"></td>
	...
	new Vue({
		...
		methods: {
			...
			async getProfitLoss(){
				...
				this.reportData = await axios.post('/get-profit-loss', this.filter).then(res => {
					return res.data;
				})
			...
			async print(){
				...
				if(this.selectedCustomer != null){
					customerText = `
						<strong>Client Id: </strong> ${this.selectedCustomer.Customer_Code}<br>
